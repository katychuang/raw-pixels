...
window.onload = function () {
    ...
    for (i = 0; i < ii; i++) {
        ...
        (function (i, val) {
            paths.push(r.path().attr({segment: [200, 200, 1, start, start + val], stroke: ""}).click(function () {
                total += data[i];
                data[i] *= 2;
                animate();
            }));
    ...
    animate(1000);
    var t = r.text(200, 20, "Click on segments to make them bigger.").attr({font: '100 20px "Helvetica Neue", Helvetica, "Arial Unicode MS", Arial, sans-serif', fill: "#888888"});
};
